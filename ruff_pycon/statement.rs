...
pub(crate) fn statement(stmt: &Stmt, checker: &mut Checker) {
    match stmt {
        ...
        Stmt::FunctionDef(
            ...
            if checker.enabled(Rule::PyConArgsWithoutType) {
                pylint::rules::pycon_args_without_type(checker, parameters);
            }
            if checker.enabled(Rule::PyConFunctionWithoutReturnDefinition) {
                pylint::rules::pycon_function_without_return_definition(checker, name, returns);
            }
        ...
        Stmt::ImportFrom(
            ...
            if checker.enabled(Rule::PyConDeprecatedTypingImport) {
                pylint::rules::pycon_deprecated_typing_import(checker, stmt, module, names);
            }
            if checker.enabled(Rule::PyConDeprecatedImport) {
                pylint::rules::pycon_deprecated_import(checker, stmt, module, names);
            }
            if checker.enabled(Rule::ModuleImportNotAtTopOfFile) {
                pycodestyle::rules::module_import_not_at_top_of_file(checker, stmt);
            }
        ...
        Stmt::Try(
            ...
            if checker.enabled(Rule::PyConAttributeError) {
                pylint::rules::pycon_attribute_error(checker, handlers);
            }
            if checker.enabled(Rule::TooManyNestedBlocks) {
                pylint::rules::too_many_nested_blocks(checker, stmt);
            }
